...
function sidebarMenuButtonVariants({
  variant = "default",
  size = "default",
}: { variant?: SidebarMenuButtonVariant; size?: SidebarMenuButtonSize } = {}) {
  return cn(SIDEBAR_MENU_BUTTON_BASE, sidebarMenuButtonVariantClasses[variant], sidebarMenuButtonSizeClasses[size]);
}
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Pseudo-random width between 50 and 90%, derived from useId so it is
  // stable across renders (Math.random() during render breaks purity rules).
  const skeletonId = React.useId();
  const width = React.useMemo(() => {
    let hash = 0;
    for (const ch of skeletonId) hash = (hash * 31 + ch.charCodeAt(0)) % 41;
    return `${hash + 50}%`;
  }, [skeletonId]);
